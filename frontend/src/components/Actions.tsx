...
const Actions: React.FC<ActionsProps> = ({ post, onReplyAdded }) => {
    ...
    // Pre-compute all color mode values at the component top level
    ...
    const replyBgColor = useColorModeValue("gray.100", "gray.700");
    const labelBgColor = useColorModeValue("rgba(211, 211, 211, 0.3)", "rgba(180, 180, 180, 0.3)");
    ...
    // Now we can have our early return AFTER all hook calls
    if (!post) return null;
    ...
    const handleReply = async (): Promise<void> => {
        ...
        try {
            ...
            // IMPORTANT: Don't modify post directly - create a new post object
            // with the updated replies array
            const updatedPost = {
                ...post,
                replies: [
                    ...(post.replies || []),
                    {
                        ...
                        image: imageBase64,
                        _id: data.replyId || Date.now().toString(), // Use server ID if provided
            ...
            // Update Redux store with the new post object
            ...
            // Fetch the updated post data without timeout
    ...
    const handleLikeAndUnlike = async (): Promise<void> => {
        if (!user)
    ...
    return (
        <>
            <Flex flexDirection={"column"}>
                <Flex justifyContent="space-between" my={2} gap={6} onClick={(e: React.MouseEvent) => e.preventDefault()}>
                    {/* SVG Like + count */}
                        ...
                        <Flex width="100%" justifyContent="space-between" alignItems="center">
                            <IconButton
                                ...
                                onClick={() => {
                                    ...
                                    fileInput?.click();      // ✅ click tồn tại trên HTMLInputElement
                                  }}
                                _hover={{
                                    bg: iconButtonBg,
                                }}
